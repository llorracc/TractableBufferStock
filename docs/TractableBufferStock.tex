...
\begin{document}
%\DeclareGraphicsExtensions{.pdf,.svg,.png,.jpg} % change to %\DeclareGraphicsExtensions{.png,.svg,.jpg} to prefer png pix
...
\end{aligned}\end{gathered}\end{equation}
which is the factor by which ${\cRat}^{e}$ would grow in the perfect foresight version of the model with permanent income growth factor $\PGro$ (again see \handoutC{PerfForesightCRRA}).  Using this, \eqref{eq:Explicit} can be written as
\begin{equation}\begin{gathered}\begin{aligned}
...
\opt{MarginNotes}{\marginpar{\tiny $\mTarg^{\hideSup}$ is point where impatience = prudence.}}
...
%\input{handoutBibMake.tex}\end{document}
...
$+$ in \eqref{eq:revshootmstart} and where appropriate in %\eqref{eq:revshootcstart})
...
For any $\mRatE_{t} \neq \mTarg^{\hideSup}$ the marginal propensity to consume can be approximated by 
\begin{equation}\begin{gathered}\begin{aligned}
...
\end{aligned}\end{gathered}\end{equation}
but at the steady state $\mRatE_{\tNow}=\mTarg^{\hideSup}$ both numerator and denominator are zero.
We get around this problem using L'H\^{o}pital's rule.  Define $\blacktriangle=\mRatE_{t}-\mTarg^{\hideSup}$ and assume that the consumption function can be approximated as
\begin{equation}\begin{gathered}\begin{aligned}
  \cFunc(\mRatE_{t}) & \approx  \cTarg^{e} + \blacktriangle_{t} \MPCE
\end{aligned}\end{gathered}\end{equation}
where 
\begin{equation}\begin{gathered}\begin{aligned}
   \MPCE & \equiv  \frac{d \cFunc(\mRatE_{t})}{d \mRatE_{t}} |_{\mRatE_{t}=\mTarg^{\hideSup}}
%\\ \MPCE^{\prime} & \equiv  \frac{d^{2} \cFunc(\mRatE_{t})}{d (\mRatE_{t})^{2}} |_{\mRatE_{t}=\mTarg^{\hideSup}}
\end{aligned}\end{gathered}\end{equation}
so that
\begin{equation}\begin{gathered}\begin{aligned}
  \blacktriangle_{t+1} & =  (\mTarg^{\hideSup} + \blacktriangle_{t} -\cFunc(\mTarg^{\hideSup}+\blacktriangle_{t}))\Rnorm + 1 - \mTarg^{\hideSup}
\\ & \approx  (\mTarg^{\hideSup}+\blacktriangle_{t}- (\cTarg^{e}+ \blacktriangle_{t} \MPCE))\Rnorm + 1 - \mTarg^{\hideSup}
\\ & =  \underbrace{(\mTarg^{\hideSup}-\cTarg^{e})\Rnorm+1}_{\mTarg^{\hideSup}} -\mTarg^{\hideSup} + (1-\MPCE) \Rnorm\blacktriangle_{t}
...
\end{aligned}\end{gathered}\end{equation}
while using \eqref{eq:ctp1Oct} yields
\begin{equation}\begin{gathered}\begin{aligned}
...
interval, which will be the MPC at $\mTarg^{\hideSup}$.  

Note that an alternative approach would be to define $\Delta \cTarg^{e}$ using the
RHS of \eqref{eq:dc} and $\Delta \mTarg^{\hideSup} = (\MPS^{e}  \Rnorm - 1)\blacktriangle_{t}$ 
and to use a numerical rootfinding routine to locate the $\hat{\MPC}^{e}$ that solves 
\begin{equation}\begin{gathered}\begin{aligned}
  \hat{\MPCFunc}^{e}(\blacktriangle_{t}) & =  \left(\frac{\Delta \cTarg^{e}}{\Delta \mTarg^{\hideSup}}\right)
\end{aligned}\end{gathered}\end{equation}
...
Differentiating \eqref{eq:naturalMPC}, dropping arguments, and evaluating at the steady state yields
\begin{equation}\begin{gathered}\begin{aligned}
